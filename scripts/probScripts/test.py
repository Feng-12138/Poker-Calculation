...
def find_combs(handCards, deskCards):
    ...
    for comb in all_combs_with_without_deskkcards:
        if (len(deskCards) == 0):
            ...
            add_to_comb = True
            for card in deskCards:
                if card not in comb or list(comb).count(card) < deskCards.count(card):
                    add_to_comb = False
            if add_to_comb:
                all_combs_include_all_desk_cards.append(comb)
...
def calculate_win_percentage(args):
    ...
    for desk in best_comb_desk_map_current[comb]:
        for comb_other in desk_best_comb_map_other[desk]:
            ...
            if result == "two":
                print("lose:", comb, comb_other)
                count_lose +=  multiplier
            else:
                print("win:", comb, comb_other)
...
if __name__ == "__main__":
    ...
    curPlayerComb, allCombs, all_combs_count_dict, all_possible_hand_card_dict = find_best_combs([12, 13], [2, 8, 13, 14, 14])
    ...
    result = calculate_prob(curPlayerComb, allCombs, all_combs_count_dict, [14, 14], best_comb_desk_map_current, desk_best_comb_map_other, best_comb_hand_card_dict, all_possible_hand_card_dict, 5)
